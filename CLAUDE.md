...
- [x] **사이트 소개(About) 페이지 신설 — AdSense 콘텐츠 품질 심사 대응** (2026-07-23) — 세션 25차 — **커밋/푸시 전, main 반영 대기**
  ...
  - `publish/about.html` — 위 렌더러로 생성한 정적 페이지 (사이트 소개·제작 방식·AI 정확성/면책 안내·운영자 연락처·관련 링크)
  - 메인 탭에는 추가하지 않고 헤더 우측 아이콘 링크로만 노출 (요청: "메인으로는 말고 상단우측탭에 링크형태로")
  - `themes/layouts/editorial.py::_layout()` hdr-actions에 About 아이콘(`_SVG_INFO`) 추가 → `render_report`/`render_ai_issue_report`/`render_archive`/`render_stock_report` 전체에 자동 반영 (다음 빌드부터)
  - `templates/header.html`(classic/ink/forest 테마 공통) `.header-actions`에도 동일 아이콘 링크 추가 — 테마 전환 시에도 일관되게 노출
  - `publish/app.html`(SPA 원본) + `publish/index.html`(컴파일 산출물) 헤더에 직접 반영
  - `vercel.json`에 `/about → /publish/about.html` 라우트 추가
  - **기존 정적 페이지 일괄 패치**: `publish/news/*.html`(133개) · `publish/ai-issue/*.html`(archive 포함, index.html 리다이렉트 스텁 제외) · `publish/stock/*.html`(index/archive 포함) 총 196개 파일에 정규식 스크립트로 About 링크 일괄 삽입 (`nav_prefix="../about.html"`, 파일 위치 기준 상대경로 — `/stock/2026-07-22` 같은 클린 URL에서도 브라우저 상대경로 해석상 정상 작동 확인)
  - **알려진 예외**: `publish/stock/archive.html` 1개 파일만 미패치 — 헤더가 구버전 레거시 구조(`id="site-nav"`, header-actions div 자체가 없음)라 자동 패치 대상에서 제외됨. 우선순위 낮음(루트 `archive.html`이 실질적 아카이브 진입점)
  - **주의**: 이 커밋은 아직 main에 push되지 않음 — 사용자 승인 후 push 예정 (push 즉시 Vercel 프로덕션에 반영됨)

- [x] **뉴스 브리핑 파이프라인 실패(2026-07-06 04:43 KST) 원인 분석 및 수정** (2026-07-06) — 세션 24차 — **PR #47 머지 완료**
  - GitHub Actions run [28752605513](https://github.com/chamgil71/dailynews/actions/runs/28752605513) 조사: RSS 수집·분석·사이트 빌드·git push까지 전부 정상 완료, "Deploy to GitHub Pages" 스텝만 GitHub 측 일시 인프라 장애(`Deployment failed, try again later.`)로 실패
  - **근본 문제**: 이 스텝에 `continue-on-error`가 없어 후속 "이메일 발송"/"텔레그램 발송" 스텝까지 통째로 스킵됨. GitHub Pages는 백업 미러일 뿐이고 실제 서비스는 Vercel(이미 git push 시점에 정상 배포)인데, 백업 배포 실패가 실제 구독자 발송까지 막아버린 것
  - `news.yml`/`ai_issue.yml`의 "Deploy to GitHub Pages" 스텝에 `continue-on-error: true` 추가 — 기존 Notion 동기화·이메일·텔레그램 스텝과 동일한 패턴으로 통일
  - `stock_build.yml`은 이메일/텔레그램을 같은 job에서 발송하지 않으므로(`stock_send.yml`이 익일 별도 처리) 수정 대상에서 제외
- [x] **AI이슈 07-05 `next_week_outlook` JSON 래퍼 파싱 재발 원인 분석 및 일반화 수정** (2026-07-06) — 세션 24차 — **PR #48 머지 완료**
  - 사용자 제보: 07-05 발행분 "차주 전망" 섹션에 raw JSON(`{"monitoring_points":[...]}`)이 그대로 노출 — CLAUDE.md 기록된 "패턴 4"의 재발이지만, 원인은 **새로운 JSON 키 이름**
  - JSON→마크다운 변환 로직이 `core/ai_issue/analyzer.py`/`core/shared/telegram.py`/`scripts/build_ai_issue_site.py`/SPA JS(`app.html`·`index.html`) 4곳에 중복 구현되어 있었고, 전부 `"points"` 키 + `commentary`/`comment` 필드명만 인식하도록 하드코딩 — Gemini가 `"monitoring_points"` 키 + `description`/`analyst_comment` 필드로 응답하자 4곳 모두 인식 실패
  - `core/shared/text_utils.py` 신설 — 리스트 키 이름을 하드코딩하지 않고 **구조(리스트-오브-딕셔너리)로 감지**하도록 일반화(`unwrap_md_wrapper`, `extract_wrapped_points`). Python 3곳은 이 공통 유틸에 위임, SPA JS는 동일 로직으로 별도 구현(Python과 공유 불가)
  - 이미 발행된 `reports/ai-issue/ai_issue_2026-07-05.md`/`.json`, `publish/ai-issue/2026-07-05.html`/`.json`의 깨진 내용도 수동 정리
  - **후속 발견(06-14 등 과거 리포트)**: 기존 로직은 `"points"` 키는 인식했지만 `description`/`analyst_comment` 필드는 못 읽고 `commentary`만 읽었음 — 과거 리포트들도 포인트 제목만 표시되고 설명 문단이 조용히 누락되고 있었음(일반화 후 재빌드 시 자동 복원됨)
- [x] **AI이슈 코드블록(`pre`) 줄바꿈 누락 CSS 버그 수정** (2026-07-06) — 세션 24차 — **PR #49 머지 완료**
  - 증상: "실용 프롬프트 / 설정 템플릿" 코드블록(개행 없는 긴 문장)이 컨테이너 폭을 넘어 한 줄로 삐져나옴
  - 원인: 프롬프트 텍스트 자체는 Gemini가 원래 개행 없이 생성한 정상 출력이고 마크다운도 올바르게 ` ```text ``` ` 블록으로 감싸져 있었음 — 문제는 `themes/base.py`(`_COMMON_CSS`)/`themes/layouts/editorial.py`/`themes/layouts/terminal.py` 3곳 모두 인라인 `code` 스타일만 있고 블록 `pre` 스타일이 아예 없어 브라우저 기본값(줄바꿈 없음)이 그대로 적용된 것
  - 3개 파일 각자의 기존 CSS 토큰(`--color-code-bg`/`--paper-2`/`--bg`+`--amber`+`--line`)에 맞춰 `pre`/`pre code`에 자동 줄바꿈(`white-space:pre-wrap`, `word-break:break-word`, `overflow-wrap:anywhere`) 규칙 추가
  - `minimal.py`는 `layout_html()`을 통해 base.py `_COMMON_CSS`를 그대로 상속하므로 별도 수정 불필요
  - 이미 발행된 AI이슈 아카이브 6개 페이지(`2026-05-31`~`2026-07-05`)도 동일 CSS로 직접 패치
- [x] **카드뉴스 SNS 발송 실패 2건 수정 (세션 23차 TODO 이행)** (2026-07-03) — **PR #46 머지 완료**
  - `post_cardnews.py::post_instagram()`: 카루셀 컨테이너 생성까지는 2207027 재시도가 있었지만 최종 `media_publish` 호출은 재시도 범위 밖이라 타이밍 에러 시 그대로 실패하던 문제 — 카루셀 컨테이너의 `FINISHED` 상태 대기 후 `media_publish`도 2207027 재시도(최대 3회, 10초 간격) 범위에 포함
  - `post_cardnews.py::post_threads()`: stock 채널 캡션이 Threads TEXT 모드 500자 제한을 넘는 날 발송 실패하던 문제 — `_truncate_caption()` 신설, 500자 초과 시 자동 축약
  - **동시 작업 충돌 실사례**: PR 작업 도중 거의 같은 시각에 병합된 **PR #44**(다른 세션)가 동일 함수(`post_instagram()`)의 카루셀 생성 단계 대기시간·재시도 횟수를 강화하는 별도 수정을 먼저 main에 반영 — `git rebase origin/main` 시 충돌 발생. 두 수정을 손으로 병합(PR #44의 강화된 대기/재시도 + 본 PR의 `media_publish` 재시도)해서 해결. **동일 파일을 여러 세션이 동시에 고칠 수 있으므로, PR을 열어둔 채 시간이 지나면 병합 전 반드시 최신 main 기준으로 리베이스/충돌 확인할 것**
  - PR 병합 후 확인한 브랜치 전수조사: `claude/*`, `fix/*` 미병합 브랜치 다수 존재하나 전부 오래된 세션 잔여물(작업은 이미 다른 경로로 main에 반영됐고 브랜치 삭제만 누락됨) — `scripts/post_cardnews.py`를 건드리는 미병합 브랜치는 없어 충돌 위험 없음 확인. 브랜치 정리는 다음 세션 TODO로 유지
- [x] **뉴스 메일링 템플릿 뉴스집계표 하단 이동 & 중복 링크 정리** (2026-06-25)
  - `templates/email_news.html`에서 상단의 뉴스집계표(`STATS BAR`) 영역을 푸터 바로 위로 이동.
  - 하단 네이비 배경인 푸터와 자연스럽게 결합되도록 집계표 패딩을 `padding: 20px 28px 0`(모바일 `16px 16px 0`)으로 조정.
  - 하단 집계표 내에 `바로가기` 단추가 포함됨에 따라, 중복되던 본문 하단의 `전체 기사 보기` 카드 영역 및 푸터 내 `웹사이트 보기 ↗` 텍스트 링크를 제거하여 메일 가독성 향상.
...
- [x] **이메일 발송 경로 통일 및 주간주식 전용 템플릿 추가** (2026-06-22) — 세션 22차 — **PR #40 머지 완료**
  - `weekly-stock`: `_send_weekly_stock()`에 `template="weekly-stock"` 누락으로 뉴스 템플릿이 사용되어 총기사 0건으로 발송되던 버그 수정
  - `templates/email_weekly_stock.html` 신규 생성 — 총평/지수성과/핫테마/전략포인트/리스크/일정 섹션
  - `mailer.py`: `_parse_md_for_weekly_stock_email()` / `_render_weekly_stock_email_template()` / `_render_ai_issue_email_template()` 추가; `send_email()` `json_data` 파라미터 및 `weekly-stock`/`ai-issue` 디스패치 추가
  - AI이슈: `send_email.py`의 사전렌더링(`html_content=`) 경로 제거 → `template="ai-issue"` / `json_data=` 통일 경로로 변경 (수신자별 HMAC 구독취소 URL 수정)
  - `email_stock.html`: 중복 `{% endif %}` 태그 제거
...
- **이메일 `template=` 디스패치 규칙**: `send_email(template=...)` — `"stock"` → `email_stock.html`, `"weekly-stock"` → `email_weekly_stock.html`, `"ai-issue"` → `email_ai_issue.html` (+ `json_data=` 필수), `None` → `email_news.html`. AI이슈는 `html_content=` 사전렌더링 경로 사용 금지 (수신자별 HMAC URL 생성 불가)
- **주간주식 MD 파싱 섹션**: `## ■ 주간 한줄 총평`, `## N. 주간 지수 성과`, `## N. 이번 주 핫 테마`, `## N. 차주 전략 포인트`, `## N. 리스크`, `## N. 차주 주요 일정`, `## 주간 온도계` — 일일 주식 MD(`## ■ 핵심 요약`, `## 시장 온도계`)와 섹션명이 다름
...
- **`themes/` 3계층 구조**: `skins/{classic,ink,forest}.py`(색상 토큰만, TOKENS dict) + `layouts/{editorial,terminal}.py`(완전 독립된 커스텀 레이아웃, base.py 미사용, 자체 CSS/HTML) + `layouts/minimal.py`(이름은 layouts/에 있지만 실제로는 `base.py::layout_html()`을 호출해 `_COMMON_CSS`를 그대로 상속하는 skin에 가까운 구조). editorial/terminal은 서로 다른 디자인 언어(신문 마스트헤드 vs 터미널 다크모드)라 CSS 변수 이름이 다른 게 정상 — 통일 대상 아님
...
### 패턴 4: "텔레그램/SPA/마크다운에 JSON이 그대로 출력된다"
- **원인**: Gemini LLM이 마크다운 텍스트 대신 JSON string 반환. **키 이름이 매번 다를 수 있음** — `{"summary":"..."}`, `{"points":[...]}`, `{"monitoring_points":[...]}` 등 (2026-07-05에 새 변형 발견)
- **진단**: `reports/ai-issue/ai_issue_YYYY-MM-DD.json`의 `next_week_outlook`, `company_trends` 필드 값 확인
- **세션 24차 이전의 함정**: 예전 수정은 `"points"`라는 **특정 키 이름 하나만** 하드코딩해서 인식했음 — 이후 Gemini가 다른 키 이름(`monitoring_points`)이나 다른 필드명(`description`/`analyst_comment`)으로 응답하면 다시 raw JSON이 노출됨. "한 번 고쳤다"가 "모든 변형에 안전하다"를 의미하지 않음 (패턴 9 참고)
- **수정(세션 24차, PR #48)**: `core/shared/text_utils.py::unwrap_md_wrapper()`/`extract_wrapped_points()` 신설 — 키 이름을 하드코딩하지 않고 **리스트-오브-딕셔너리 구조로 감지**, point/detail 후보 필드명도 여러 개 순차 시도. `core/ai_issue/analyzer.py`/`core/shared/telegram.py`/`scripts/build_ai_issue_site.py` 3곳이 이 공통 유틸에 위임. SPA JS(`app.html`·`index.html`의 `extractMdField()`)는 Python과 공유 불가하므로 동일 로직을 별도 구현 — **새 JS 코드도 반드시 4곳 전부(위 3개 Python + JS 2파일) 동시 수정할 것, 하나만 고치면 나머지에서 재발함**
- **예방**: `core/ai_issue/analyzer.py`에서 LLM 응답 파싱 시 JSON/markdown 양방향 처리(근본 원인 위치 — 여기서 막히면 `.md`/`.json`에 애초에 깨끗한 텍스트만 저장됨)
...
### 패턴 11: "파이프라인 실패 알림이 왔는데 실제로는 콘텐츠가 다 정상 생성됐다"
- **원인**: `news.yml`/`ai_issue.yml`은 수집→분석→빌드→git push→**GitHub Pages 배포**→이메일→텔레그램 순서로 한 job 안에서 순차 실행됨. GitHub Pages는 Vercel(메인 배포)과 무관한 **백업 미러**인데, 이 배포 스텝에 `continue-on-error`가 없으면 GitHub 측 일시 장애만 나도 뒤의 이메일/텔레그램 스텝까지 전부 스킵됨 (GitHub Actions 기본 동작: 이전 스텝 실패 시 `if: always()` 없는 후속 스텝은 스킵)
- **진단**: 실패한 run의 job 로그에서 정확히 어느 스텝이 실패했는지 확인. "Deploy to GitHub Pages"에서 `Deployment failed, try again later.` 같은 GitHub 자체 인프라 메시지가 보이고 그 앞 스텝(수집/분석/빌드/git push)이 전부 성공이면 콘텐츠 자체는 문제 없음
- **수정(세션 24차, PR #47)**: `news.yml`/`ai_issue.yml`의 "Deploy to GitHub Pages" 스텝에 `continue-on-error: true` 추가 (기존 Notion 동기화·이메일·텔레그램 스텝과 동일 패턴). `stock_build.yml`은 같은 job에서 이메일/텔레그램을 발송하지 않으므로 해당 없음
- **주의**: 이 수정 이후로는 GitHub Pages 배포만 실패해도 job 전체 상태가 success로 표시됨(continue-on-error가 conclusion을 success로 마스킹) — Pages 자체가 진짜 안 되는지 별도로 확인하려면 Actions 로그를 직접 봐야 함

### 패턴 12: "코드블록(프롬프트/설정 템플릿 등)이 줄바꿈 없이 한 줄로 삐져나온다"
- **원인**: 개행 없는 긴 문장이 마크다운 ` ```text ``` ` 코드블록으로 감싸질 때, `themes/base.py`(`_COMMON_CSS`)/`themes/layouts/editorial.py`/`themes/layouts/terminal.py` 3곳 모두 인라인 `code` 스타일만 있고 블록 `pre` 스타일이 없어서 브라우저 기본값(`white-space:pre`, 줄바꿈 없음)이 그대로 적용됨. LLM이 잘못 응답한 게 아니라 **CSS 누락**이 원인
- **진단**: 해당 섹션이 코드블록(` ``` `)으로 감싸진 자연어 텍스트인지 확인. 마크다운 소스 자체엔 문제 없고 렌더링된 HTML에서 `<pre>`가 컨테이너 폭을 벗어나면 이 패턴
- **수정(세션 24차, PR #49)**: 3개 파일 각자의 기존 CSS 토큰(`--color-code-bg`/`--paper-2`/`--bg`+`--amber`+`--line`)에 맞춰 `pre`/`pre code`에 `white-space:pre-wrap; word-break:break-word; overflow-wrap:anywhere;` 추가. `minimal.py`는 `layout_html()`로 base.py `_COMMON_CSS`를 상속하므로 별도 수정 불필요
- **주의**: `editorial`/`terminal`은 `classic`/`ink`/`forest`와 달리 **의도적으로 완전히 독립된 커스텀 레이아웃**(다른 HTML 구조·폰트·디자인 언어)이라서 CSS 변수 이름이 다름 — 통일 대상이 아님. 향후 이 3개 파일에 공통 CSS 규칙을 추가할 일이 생기면 반드시 3곳 전부 각자의 토큰에 맞게 수정할 것

### 패턴 13: "PR을 열어뒀는데 나중에 보니 다른 세션이 같은 파일을 먼저 고쳐서 main에 병합했다"
- **원인**: Claude Code 웹은 세션마다 독립된 브랜치에서 작업하므로, 같은 파일(특히 반복적으로 실패하는 `post_cardnews.py` 같은 파일)을 서로 다른 시각에 시작된 두 세션이 동시에 고칠 수 있음. 먼저 병합된 쪽이 base를 바꿔버리면 나중 PR은 `mergeable_state`가 `dirty`(진짜 충돌)로 바뀜. draft PR을 오래 열어두거나, PR을 열고 한참 뒤에 병합할수록 이 확률이 올라감
