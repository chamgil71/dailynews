...
- [ ] 이메일 발송 시 푸터 "구독 취소" 링크 정상 렌더링 + 클릭 시 Supabase unsubscribed 처리 확인
- [ ] `cardnews.yml` 수동 실행 → Instagram/Threads/Facebook/Telegram 모두 정상 발송 확인
- [ ] `TELEGRAM_CHAT_ID_STOCK` 형식 확인 (`-1001003979260668` 맞는지)
- [ ] `NOTION_DATABASE_ID_STOCK` / `NOTION_DATABASE_ID_AI_ISSUE` 실제 값으로 교체 필요
- [ ] `news.yml` 다음 자동 실행 — Vercel search-index 업데이트 포함 여부 확인
