...
### 진행 중 (분석 완료, 수정 미착수) — 세션 23차 (2026-07-01)
- [ ] **카드뉴스 SNS 발송 실패 원인 분석 완료** (2026-07-01) — 수정은 다음 세션에서 진행
  - GitHub Actions API(`GH_CONTENTS_TOKEN`)로 `cardnews.yml` 최근 실행 로그 직접 조회해 원인 특정. **Facebook·Telegram은 실패 없음, 뉴스/AI이슈 채널도 정상.** 실패는 전부 **stock 채널**에서만 발생.
  - **어제(06-30, stock 06-29 리포트) Instagram 실패**: `error_subcode 2207027 "Media ID is not available"`. 컨테이너 생성·FINISHED 대기까지는 통과하고 **`media_publish`(최종 게시) 호출 단계**에서 발생 — 이 호출은 기존 2207027 재시도 로직(`post_cardnews.py::post_instagram()`) 범위 밖이라 재시도가 아예 안 됨 (로그에 재시도 메시지 없음으로 확인). 기존 재시도는 카루셀 **컨테이너 생성** 호출만 감쌈 — **사각지대**.
  - **오늘(07-01, stock 06-30 리포트) Threads 실패**: `"Param text must be at most 500 characters long."`. `_build_caption()`의 stock 캡션(🌡시장온도+📌요약 2줄+🔑키워드 3개+🔗링크+해시태그, 세션 20차에서 추가)에 500자 상한 체크가 없어서 요약이 긴 날 초과.
  - **다음 세션 TODO**:
    1. `post_cardnews.py::post_instagram()` — `media_publish` 호출도 2207027 재시도 범위에 포함
    2. `post_cardnews.py::_build_caption()` — stock 캡션을 Threads TEXT 모드 500자 제한 이내로 자르는 로직 추가 (또는 Threads 전용으로 캡션 축약)
  - 참고: GitHub Actions 로그 조회 방법 — `gh` CLI 미설치 상태였음. `.env`의 `GH_CONTENTS_TOKEN`으로 `curl --ssl-no-revoke -H "Authorization: Bearer $TOKEN" https://api.github.com/repos/chamgil71/dailynews/actions/...` 직접 호출로 대체 (Windows curl+schannel 조합에서 Azure Blob 리다이렉트 시 `CRYPT_E_NO_REVOCATION_CHECK` 에러 발생 — `--ssl-no-revoke` 필수)
