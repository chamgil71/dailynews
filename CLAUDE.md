...
- [ ] 6/16(화) `stock_send.yml` KST 08:00 — `stock_2026-06-15.md` 발송 확인 (3일 이내 체크)
- [ ] 6/16(화) 클로드 루틴 Step 0 동작 확인 — Python으로 월요일 정상 감지 후 루틴 진행
- [ ] `cardnews.yml` 다음 stock 실행 — `stock_send.yml` 완료 후 트리거되는지 확인 (텍스트→카드뉴스 순서)
- [ ] Threads stock 캡션에 🌡 시장온도·📌 요약·🔑 키워드 포함되는지 확인 (세션 20차)
- [ ] news/ai-issue Threads 캡션에 issue_titles(제목 3개) 포함되는지 확인 (세션 20차)
...
- **`core/shared/alert.py`**: 3채널 공통 실패 알림 모듈. `send_pipeline_alert(channel, date_str, reason)` — 텔레그램+관리자 이메일(`mailer.send_admin_alert()` 재사용) best-effort 발송. `run_news.py`/`run_stock.py`/`run_ai_issue.py`가 각자 구현하던 SMTP/텔레그램 코드를 대체 (세션 19차)
- **`core/shared/report_date.py`**: KST 날짜 계산 공통 모듈. `kst_today()`/`kst_now()` — `datetime.now()`(OS TZ 환경변수 의존, GitHub Actions의 `env: TZ: Asia/Seoul`에 암묵적으로 의존하던 패턴)를 대체해 타임존을 코드에 명시. `mailer.py`/`telegram.py`/`run_*.py`/`send_*.py` 전체가 이걸 사용 (세션 19차)
- **뉴스 품질 게이트 위치**: `analysis_ok` 체크가 이메일(`send_email.py::_send_news`)·텔레그램(`send_telegram.py::_send_news`)에는 있었지만 사이트 빌드에는 없었음 → `news.yml`에 `분석 결과 확인` 스텝(`id: check_analysis`) 추가, `사이트 빌드`/`Notion 뉴스 동기화` 스텝의 `if:` 조건에 `steps.check_analysis.outputs.ok != 'false'` 추가 (세션 19차, 실행 미확인 — 검증 필요 표 참조)
- **카드뉴스 `data.json` extra 필드**: `_update_index(extra_data=)` 파라미터로 채널별 추가 데이터 저장. news: `issue_titles`(top3 제목), ai-issue: `issue_titles`(top3), stock: `summary`/`keywords`/`temperature`. `post_cardnews.py::_build_caption()`에서 채널 분기로 활용 (세션 20차)
- **카드뉴스 발송 순서 (stock)**: `stock_build.yml`(밤 빌드) → `stock_send.yml`(08:00 텍스트 텔레그램) → `cardnews.yml` 트리거(카드뉴스 이미지). `cardnews.yml` 트리거가 `"Stock Briefing Send"`임에 주의 — `"Stock Briefing Build"`가 아님 (세션 20차)
