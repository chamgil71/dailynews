  ...
  <div class="wrap">
    <div class="masthead">
      ...
      <a href="{site_url}" style="text-decoration:none;color:inherit">
        <h1 class="mh-title">AI <em>News</em> Daily</h1>
      </a>
    ...
    <div class="foot">
      <span>AI News Daily · {site_title}</span>
...
def render_archive(ctx: dict) -> str:
    ...
    stock_html = "".join(f"""
        <li>
          <a href="stock/{it['date']}.html">{it['display']}</a>
          <div class="d">{it['date']}</div>
        </li>""" for it in stock_items)
    # AI이슈 목록
    ai_items = ctx.get("ai_items", [])
    ai_html = "".join(f"""
        <li>
          <a href="ai-issue/{it['date']}.html">{it['display']}</a>
          <div class="d">{it['date']}</div>
        </li>""" for it in ai_items)
    # 통계 헤더
    total = len(ctx['items']) + len(stock_items) + len(ai_items)
    stats_html = f"""
    <div class="arc-stats" style="display:flex;gap:16px;flex-wrap:wrap;margin-bottom:24px;padding:16px;background:var(--bg);border:1px solid var(--rule);border-radius:6px">
      <div><strong style="font-size:1.3rem">{len(ctx['items'])}</strong><div style="font-size:.75rem;color:var(--muted);text-transform:uppercase;letter-spacing:.08em">뉴스 리포트</div></div>
      <div><strong style="font-size:1.3rem">{len(stock_items)}</strong><div style="font-size:.75rem;color:var(--muted);text-transform:uppercase;letter-spacing:.08em">주식 시황</div></div>
      <div><strong style="font-size:1.3rem">{len(ai_items)}</strong><div style="font-size:.75rem;color:var(--muted);text-transform:uppercase;letter-spacing:.08em">AI 이슈</div></div>
      <div><strong style="font-size:1.3rem">{total}</strong><div style="font-size:.75rem;color:var(--muted);text-transform:uppercase;letter-spacing:.08em">전체</div></div>
    </div>"""
    # 탭 패널
    body = f"""
    <div class="arch">
      ...
      <div style="display:flex;gap:8px;margin-bottom:16px;flex-wrap:wrap">
        <button onclick="showTab('news')"    id="tabNews"    style="padding:6px 14px;border:1px solid var(--rule);border-radius:4px;background:var(--accent);color:#fff;cursor:pointer;font-size:.85rem">📰 뉴스 ({len(ctx['items'])})</button>
        <button onclick="showTab('stock')"   id="tabStock"   style="padding:6px 14px;border:1px solid var(--rule);border-radius:4px;background:var(--bg);cursor:pointer;font-size:.85rem">📈 주식 ({len(stock_items)})</button>
        <button onclick="showTab('ai')"      id="tabAI"      style="padding:6px 14px;border:1px solid var(--rule);border-radius:4px;background:var(--bg);cursor:pointer;font-size:.85rem">🤖 AI이슈 ({len(ai_items)})</button>
      </div>
      <div id="tabPanelNews"  ><ul>{news_items}</ul></div>
      <div id="tabPanelStock" style="display:none"><ul>{stock_html or "<li style='color:var(--muted);padding:12px'>주식 리포트 없음</li>"}</ul></div>
      <div id="tabPanelAI"    style="display:none"><ul>{ai_html or "<li style='color:var(--muted);padding:12px'>AI 이슈 보고서 없음</li>"}</ul></div>
    </div>
    <script>
    function showTab(t){{
      ['news','stock','ai'].forEach(k=>{{
        document.getElementById('tabPanel'+k.charAt(0).toUpperCase()+k.slice(1)).style.display=k===t?'':'none';
      }});
      document.getElementById('tabNews').style.background  = t==='news'  ? 'var(--accent)' : 'var(--bg)';
      document.getElementById('tabStock').style.background = t==='stock' ? 'var(--accent)' : 'var(--bg)';
      document.getElementById('tabAI').style.background    = t==='ai'    ? 'var(--accent)' : 'var(--bg)';
      document.getElementById('tabNews').style.color  = t==='news'  ? '#fff' : 'inherit';
      document.getElementById('tabStock').style.color = t==='stock' ? '#fff' : 'inherit';
      document.getElementById('tabAI').style.color    = t==='ai'    ? '#fff' : 'inherit';
    }}
    </script>"""
