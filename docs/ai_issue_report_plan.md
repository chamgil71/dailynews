...
jobs:
  ai_issue:
    ...
    steps:
      ...
      - name: Telegram 발송
        run: python scripts/send_ai_issue_telegram.py
        env:
...
## 8. Telegram 발송 스크립트 (`scripts/send_ai_issue_telegram.py`)

```python
"""주간 AI 이슈 Telegram 요약 발송"""
import json, sys
...
def main():
    ...
    json_path = Path(_ROOT) / "reports" / "ai-issue" / f"ai_issue_{today}.json"
    if not json_path.exists():
        print(f"[Telegram] 파일 없음: {json_path}")
        return

    data = json.loads(json_path.read_text(encoding="utf-8"))
    period = data.get("period", "")
    top3 = data.get("top10", [])[:3]
    outlook = data.get("next_week_outlook", "")

    lines = [f"🤖 *AI Issue Weekly* — {period}", ""]
    for item in top3:
        rank = item.get("rank", "")
        title = item.get("title", "")
        cat = item.get("category", "")
        lines.append(f"{rank}. [{cat}] {title}")
    if outlook:
        lines += ["", f"🔮 차주 전망: {outlook[:100]}..."]
...
## 10. 이메일 템플릿 (`email_ai_issue.html`)
...
| `core/shared/telegram.py` | `send_message()` 그대로 |
