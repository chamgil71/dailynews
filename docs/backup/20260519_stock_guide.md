...
push trigger (reports/stock/**.md)
  → send_stock_email + build_stock_site + build_site + commit + deploy

schedule/workflow_dispatch(mode=full)
  → stock_main (수집+분석+저장) + send_stock_email + build + commit + deploy

workflow_dispatch(mode=build_only)
  → build_stock_site + build_site + commit + deploy
```
