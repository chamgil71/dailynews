...
8. [이메일/텔레그램 발송 (send_email.py / send_telegram.py)](#8-이메일-발송)
...
scripts/
  ...
  send_email.py            ← 통합 이메일 발송 (--type stock)
  send_telegram.py         ← 통합 텔레그램 발송 (--type stock)
...
3. `scripts/send_telegram.py --type stock` — 텔레그램 `@msstockbrief` 발송 (`continue-on-error: true`)
...
| 경로 | 스크립트 | 비고 |
|------|---------|------|
| 이메일 발송 | `scripts/send_email.py --type stock` | 품질 게이트 포함, 리포트 날짜 기준 요일 레이블 |
| 텔레그램 발송 | `scripts/send_telegram.py --type stock` | `@msstockbrief` 채널 |

### scripts/send_email.py --type stock

```python
# send_email.py _send_stock()
...
ok = send_email(md_path.read_text(), template="stock", subject_override=subject,
                report_date=date_str, channel="stock")
```
...
schedule/workflow_dispatch(mode=full)
  → stock_main (수집+분석+저장) + build + commit + deploy (Pages)
...
이메일은 중복 발송될 수 있으므로, 필요 시 `send_email.py`에 날짜 체크 추가.
