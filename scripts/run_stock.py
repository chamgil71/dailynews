...
def main() -> None:
    ...
    # ── 주말 체크 ─────────────────────────────────────────────────────────────
    # 토·일에는 거래가 없으므로 실행 자체를 중단한다.
    # 주간 루틴(토요일 수동 실행)은 run_stock.py를 거치지 않고 별도 루틴으로 처리.
    if now_weekday >= 5:
    ...
    date_str = stock_data.get("trading_date", kst_today())
    ...
    if not analysis_ok:
        ...
        send_pipeline_alert("stock", date_str,
